...
def mw_2_volts(mw, resistance):

    amplitude_rms = np.sqrt((mw/1000)*resistance)
    amplitude_peak = amplitude_rms * np.sqrt(2)

    return [amplitude_peak, amplitude_rms]


def export_as_pwl(name, time, signal):
    file = open(name,"w")

    for local_index in range(0, len(signal)):
        file.write(str(time[local_index]))
        file.write(" ")
        file.write(str(signal[local_index]))
        file.write("\n")
    file.close()
